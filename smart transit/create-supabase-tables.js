// Create Supabase tables with the correct connection format
...
if (!dbUrl || dbUrl.includes('[YOUR-PASSWORD]')) {
  console.log('❌ Please update your .env file with your actual Supabase password')
  console.log('Replace [YOUR-PASSWORD] with your real password in the DATABASE_URL')
  process.exit(1)
}
...
async function createTables() {
  ...
  try {
    ...
    const test = await sql`SELECT NOW() as current_time, version() as version`
    console.log('✅ Connected! Server time:', test[0].current_time)
    console.log('🗄️  Database:', test[0].version.split(' ')[0])
    ...
    // Sample buses
    await sql`INSERT INTO bus_info (bus_id) VALUES ('B1'), ('B2')`
    ...
    // Test data
    const users = await sql`SELECT user_id, name, email, balance FROM user_profile`
    console.log('👥 Sample users:', users)
    
    const buses = await sql`SELECT * FROM bus_info`
...
async function main() {
  console.log('🚀 Smart Transit - Supabase Table Creation\n')
  ...
  if (success) {
    ...
    console.log('📱 You can now update your API to use real database queries')
  } else {
    console.log('\n❌ Database setup failed.')
    console.log('💡 Try using the Supabase SQL Editor instead')
  }
}
